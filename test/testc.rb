...
#  f = fairy.input(["file://emperor//home/keiju/public/a.research/fairy/git/fairy/sample/wc/data/sample_30M.txt"]*60)
  f = fairy.input(["file://emperor//home/keiju/public/a.research/fairy/git/fairy/sample/wc/data/sample_30M.txt"]*240)
...
#  f = fairy.input(["file://giant/home/keiju/public/a.research/fairy/git/fairy/sample/wc/data/fairy.cat"]*1)
  ...
  f = f.mod_group_by(%{|w| w},
		     :postqueuing_policy => {:queuing_class => :ChunkedFileBufferdQueue},
		     :buffering_policy => {
		       :buffering_class => :CommandMergeSortBuffer,
                       :threshold => 10000,
                     })
  f = f.map(%{|key, values| [key, values.size].join(" ")})
  #  f.here.each{|e| puts e.join(" ")}
...
#  f = fairy.input(["file://giant/home/keiju/public/a.research/fairy/git/fairy/sample/wc/data/fairy.cat"]*1)
  ...
  f = f.mod_group_by(%{|w| w},
		     :postqueuing_policy => {:queuing_class => :ChunkedFileBufferdQueue},
		     :buffering_policy => {
		       :buffering_class => :OnMemoryBuffer
                     })
  f = f.map(%{|key, values| [key, values.size].join(" ")})
  #  f.here.each{|e| puts e.join(" ")}
  f.output("test/test-66.vf")
